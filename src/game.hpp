 ...
 * and you probably want to use that allocator instead of the default
 * ones */
...
/* We can re-use the types provided by SDL which are extremely
 * portable, so there is no need for Nuklear to detect those on its
 * own */
...
/* We can reuse the `bool` symbol because SDL3 guarantees its
 * existence */
...
/* SDL can also provide us with math functions, but beware that
 * Nuklear's own implementation can be slightly faster at the cost of
 * some precision */
 ...
 * those libraries pull in some libc headers internally, creating a
 * linkage dependency, so you’ll most likely want to use SDL symbols
 * instead */
...
static const std::filesystem::path EXE_DIR =
    get_ExecutableDirectory();
...
class Game {
   public:
    ...
    ~Game() {
        nk_input_end(ctx);
        nk_sdl_shutdown(ctx);
        for (int i = deletors.size(); i > 0; --i) {
            deletors.top()();
            deletors.pop();
        }
        // Let caller release SDL resource
        window   = nullptr;
        renderer = nullptr;
    }

    static constexpr std::string_view main_bg_src =
        "res/textures/main_menu_background.jpg";
    static constexpr SDL_Color BACKGROUND_COLOR = {0, 150, 130, 255};
    ...
    nk_context*             NkContext() { return ctx; }
   ...
   private:
    // the class doesn't own the SDL resource
